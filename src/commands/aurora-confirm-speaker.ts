...
export async function auroraConfirmSpeakerCommand(voicePrintId: string, identityName: string): Promise<void> {
  try {
    ...
    const existing = identities.find(i => i.name.toLowerCase() === identityName.toLowerCase());
    if (existing) {
      ...
      console.log(chalk.green(`  ✅ Confirmed: ${voicePrintId} is "${result.identity.name}"`));
      ...
      console.log(chalk.green(`  ✅ Confirmed: ${voicePrintId} is "${identity.name}"`));
