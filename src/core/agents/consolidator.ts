...
`;
   ...
   * Find pairs of nodes with similar titles that may be duplicates.
   */
  private async executeFindDuplicateCandidates(input: {
    ...
    await this.ctx.audit.log({
      ...
      allowed: true,
      note: `Finding duplicates with threshold ${threshold}`,
    });

    const graph = await loadGraph(graphPath);
    const candidates = findDuplicateCandidates(graph, threshold);
